...
Future<LottoNumberModel> fetchWinningLottoNumbers(List<int> numbers) async {
  ...
  if(response.statusCode == 200) {
    List<List<dynamic>> list = List.from(List<dynamic>.from(json.decode(response.body)));
    // 현재는 한건씩 처리하는 로직으로 되어 있어 단건씩 넣음
    if(list.isNotEmpty) {
      // result
      LottoNumberModel model = LottoNumberModel.fromJson(list[0].map((e) => e.toInt()).toList().toString());
      return model;
    }
