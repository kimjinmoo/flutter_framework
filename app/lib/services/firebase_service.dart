...
final commandRef = FirebaseFirestore.instance
    .collection("comments")
    .withConverter<CommandsModel>(
      fromFirestore: (snapshots, _) =>
          CommandsModel.fromJson(snapshots.data()!),
...
final userLottoRef = FirebaseFirestore.instance
    .collection("lotto")
    .withConverter<UserLottoModel>(
      fromFirestore: (snapshots, _) =>
          UserLottoModel.fromJson(snapshots.data()!),
      toFirestore: (snapshots, _) => snapshots.toJson(),
    );
  ...
  Query<CommandsModel> queryBy(CommandsQuery query) {
    switch (query) {
      ...
      case CommandsQuery.createDateDesc:
        return orderBy('regDate',
            descending: query == CommandsQuery.createDateDesc);
    }
  }
}
...
Future<UserModel> updateUserName(String userId, String userName) async {
  ...
  if (user.docs.isNotEmpty) {
    ...
    QuerySnapshot<CommandsModel> commands =
        await commandRef.where("userId", isEqualTo: userId).get();
...
Future<UserModel> updateUserMaxRank(String userId, int maxRank) async {
  ...
  if (user.docs.isNotEmpty) {
    ...
    if (userModel.maxRank == 0 || userModel.maxRank > maxRank) {
      UserModel updatedUserModel = UserModel(
          userId: userModel.userId,
...
Future<void> addCommand(commandsModel) async {
  commandRef.add(commandsModel).then((value) => print(value));
}
...
Future<void> updateCommand(widget) async {
  int updated =
    ...
    DocumentSnapshot<CommandsModel> command =
        await transaction.get<CommandsModel>(widget.reference);
    return 0;
  });
}
...
Future<void> reportComment(id) async {
  return commandRef.doc(id).update({'isReport': "Y"});
}
...
Future<UserModel?> getUser(String id) async {
  QuerySnapshot<UserModel> querySnapshot =
      await userRef.where("userId", isEqualTo: id).get();
...
Future<void> addUser(UserModel userModel) async {
  userRef.add(userModel).then((value) => print(value));
}

/// 내 추천점호를 가져온다.
Future<UserLottoModel> fetchMyLotto(int round, String userId) async {
  QuerySnapshot<UserLottoModel> querySnapshot = await userLottoRef
      .where("round", isEqualTo: round)
  ...
  return UserLottoModel(
      numbers: [], round: 0, regDate: Timestamp.now(), maxRank: 0, userId: "");
}
...
Future<void> deleteMyLotto(int round, String userId, int removeIndex) async {
  ...
  if (querySnapshot.docs.isNotEmpty) {
    ...
    if (!numbers.isEmpty) {
      ...
      userLottoRef
          .doc(id)
          .update({'numbers': numbers.map((e) => e.toJson()).toList()});
    }
  }
}
...
Future<void> addMyLotto(UserLottoModel userLottoModel) async {
  QuerySnapshot<UserLottoModel> querySnapshot = await userLottoRef
      .where("round", isEqualTo: userLottoModel.round)
      .where("userId", isEqualTo: userLottoModel.userId)
      .get();
  if (querySnapshot.docs.isNotEmpty) {
    // ID를 가져온다.
    var id = querySnapshot.docs[0].reference.id;
